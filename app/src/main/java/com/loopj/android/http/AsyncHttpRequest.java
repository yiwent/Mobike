...
public class AsyncHttpRequest implements Runnable {
     ...
     * processed by the system. The library makes sure that getUrl single request
     ...
     * sent, handled and finished. The library makes sure that getUrl single request
        ...
        try {
            while (retry) {
                try {
                    ...
                    // switching between WI-FI and mobile data networks can cause getUrl retry which then results in an UnknownHostException
                    // while the WI-FI is initialising. The retry logic will be invoked here, if this is NOT the first retry
                    // (to assist in genuine cases of unknown host) which seems better than outright failure
                    cause = new IOException("UnknownHostException exception: " + e.getMessage());
                    retry = (executionCount > 0) && retryHandler.retryRequest(cause, ++executionCount, context);
                } catch (NullPointerException e) {
                    // there's getUrl bug in HttpClient 4.0.x that on some occasions causes
